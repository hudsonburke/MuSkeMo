...
from ..inertial.presets import InertialPropertiesPresets
...
class MuSkeMoProperties(PropertyGroup):
    ...
    segment_parameter_list_arithmetic: CollectionProperty(
        type=SegmentParameterItem
    )  # imported class from the inprop panel script
    segment_parameter_list_logarithmic: CollectionProperty(
        type=SegmentParameterItem
    )  # for logarithmic per segment expansion
    whole_body_mass_logarithmic_parameters: CollectionProperty(
        type=SegmentParameterItem
    )  # for logarithmic summed ch volume to estimate total body mass
    segment_inertial_logarithmic_parameters: CollectionProperty(
        type=SegmentParameterItem
    )  # for per segment log ch parameters to directly estimate inertial properties of the segment
    ...
    def update_expansion_template_arithmetic(self, context):
        ...
        if preset_key == "Custom":
            ...
            # Use preset data
            preset_data = InertialPropertiesPresets["Arithmetic scale factor"].get(
                preset_key, ([], [])
            )
    ...
    def update_expansion_template_logarithmic(self, context):
        ...
        if preset_key == "Custom":
            ...
            # Use preset data
            preset_data = InertialPropertiesPresets["Logarithmic scale factor"].get(
                preset_key, ([], [], [])
            )
    ...
    def update_mass_from_CH_template_logarithmic(self, context):
        ...
        if preset_key == "Custom":
            ...
            # Use preset data
            preset_data = InertialPropertiesPresets["Logarithmic whole body mass"].get(
                preset_key, ([], [], [])
            )
            body_segments, factors1, factors2, factors3 = preset_data
            for i, segment in enumerate(body_segments):
    ...
    def update_segment_inprops_from_CH_template_logarithmic(self, context):
        ...
        if preset_key == "Custom":
            ...
            # Use preset data
            preset_data = InertialPropertiesPresets[
    ...
    expansion_template_arithmetic: EnumProperty(
        ...
        + [
            (key, key, "")
            for key in InertialPropertiesPresets["Arithmetic scale factor"].keys()
        ],
    ...
    expansion_template_logarithmic: EnumProperty(
        ...
        + [
            (key, key, "")
            for key in InertialPropertiesPresets["Logarithmic scale factor"].keys()
        ],
        default="Custom",  # Ensure Custom is default
    ...
    mass_from_CH_template_logarithmic: EnumProperty(
        ...
        + [
            (key, key, "")
            for key in InertialPropertiesPresets["Logarithmic whole body mass"].keys()
        ],
        default="Custom",  # Ensure Custom is default
    ...
    segment_inprops_from_CH_template_logarithmic: EnumProperty(
        ...
        + [
            (key, key, "")
            for key in InertialPropertiesPresets[
                "Logarithmic segment inertial properties"
            ].keys()
        ],
        default="Custom",  # Ensure Custom is default
    ...
    segment_index: IntProperty(
        name="Active Segment Index",  # this is used internally by the seg inprops from CH panel. THat is such a long list that it should be in a scrollable box in the panel, and that requires a UIList which needs to be passed the index as a property
        default=-1,
    )
    ...
    frame_construction_mode: EnumProperty(
        ...
        items=[
            ("X-Yt", "Specify X-axis and Y-temp direction (for XY plane)", ""),
    ...
    trajectory_filetype: EnumProperty(
        ...
        items=[
            ("OpenSim (.sto or .mot)", "OpenSim (.sto or .mot)", ""),
            ("Custom", "Custom", ""),
        ],
    ...
    icp_alignment_mode: EnumProperty(
        ...
        items=[
            ...
            (
                "Selected mesh portions",
                "Selected mesh portions",
                "Align selected mesh portions",
            ),
        ],
