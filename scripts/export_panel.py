...
from bpy.props import (
    StringProperty,  # it appears to matter whether you import these from types or from props
    BoolProperty,
)
...
class ExportHelperCustom:  ## this is a helper class, that is inherited by all the actual export operators below. This prevents reusing code.
    ...
    def check(self, context):  ### ensure the file extension is not changed by the user
        ...
        if os.path.basename(filepath):
            filepath = bpy.path.ensure_ext(
                os.path.splitext(filepath)[0], self.filename_ext
            )
    ...
    def invoke(
        self, context, event
    ):  ## set a filepath using a subclass dependent default filename, and filter out other filetypes from the viewer
        ...
        if not model_export_dir:  ### if the user-selected export directory is empty, it defaults to the blend file's parent directory
            ...
            model_export_dir = (
                os.path.split(model_export_dir)[0] + "\\" + default_filename
            )

        self.filename_ext = (
            "." + bpy.context.scene.muskemo.export_filetype
        )  # user assigned
        self.filepath = (
            model_export_dir + self.filename_ext
        )  # this sets the filepath, and the file extension.

        self.filter_glob = (
            "*" + self.filename_ext
        )  # set the filetype filter in the export window
        ...
        number_format = (
            bpy.context.scene.muskemo.number_format
        )  # can be 'e', 'g', or '8f'
        ...
        self.number_format = (
            "." + number_format
        )  # number format needs to be something like '.4e'
...
class SelectModelExportDirectoryOperator(Operator):
    ...
    bl_description = (
        "Select the directory where you would like to export your model files to"
    )
    ...
    directory: StringProperty(
        name="Outdir Path", description="User selected output directory"
    )
...
class ExportBodiesOperator(
    Operator, ExportHelperCustom
):  # inherits from ExportHelperCustom class
    ...
    def invoke(self, context, event):
        # set the default filename to the collection name, make it available for the "invoke" command of super class "exporthelpercustom"
        self.default_filename = bpy.context.scene.muskemo.body_collection
        ...
        for joint in joints:
            if "MuSkeMo_type" not in joint:
                self.report(
                    ...
                    + "' in the '"
                    + joint_colname
                    + "' collection was not created by MuSkeMo, remove it from the collection first. Operation cancelled",
                )
                return {"FINISHED"}
            ...
            if joint["MuSkeMo_type"] != "JOINT":
                self.report(
                    ...
                    + "' in the '"
                    + joint_colname
                    + "' collection is not a JOINT, remove it from the collection first. Operation cancelled",
                )
                return {"FINISHED"}
            ...
            if joint["parent_body"] == "not_assigned":
                self.report(
                    ...
                    "Joint with the name '"
                    + joint.name
                    + "' has no parent body assigned. If you use one of MuSkeMo's conversion scripts, this joint will be treated as a root joint (its parent will be 'ground', 'World', etc., depending on the simulator)",
                )

        write_joints(
            context, self.filepath, joint_colname, delimiter, self.number_format, self
        )
        return {"FINISHED"}
...
class ExportMusclesOperator(
    ...
    def execute(self, context):
        ...
        write_muscles(
            context, self.filepath, muscle_colname, delimiter, self.number_format
        )
        return {"FINISHED"}
...
class ExportMeshInPropsOperator(
    Operator, ExportHelperCustom
):  # inherits from ExportHelperCustom class
    ...
    def invoke(self, context, event):
        ...
        if not self.default_filename:  # if the target collection has not been named
            self.report(
                {"ERROR"},
                "You did not designate a collection (Blender folder) in which the 3D meshes are located. Place them in a single collection, and type the name in the text box",
            )
            return {"FINISHED"}
...
class ExportFramesOperator(
    ...
    def execute(self, context):
        ...
        write_frames(
            context, self.filepath, frame_colname, delimiter, self.number_format
        )
        return {"FINISHED"}
...
class ExportGeometryFolderOperator(Operator):
    ...
    def execute(self, context):
        ...
        if not model_export_directory:
            self.report(
                {"ERROR"},
                "You must first select a model export directory. Press the 'Select export directory' and choose a target folder",
            )
            return {"FINISHED"}
        ...
        objects = [
            x
            for x in bpy.data.collections[geo_folder].objects
        ...
        for obj in objects:
            ...
            self.report(
                {"INFO"},
                "Exported geometry with the name '"
                + name
                + "' to the '"
                + geo_folder
                + "' subdirectory",
...
## Main export panel
class VIEW3D_PT_export_panel(
    VIEW3D_PT_MuSkeMo, Panel
):  # class naming convention ‘CATEGORY_PT_name’
    bl_idname = (
        "VIEW3D_PT_export_panel"  # have to define this if you use multiple panels
    )
    ...
    def draw(self, context):
        ...
        row.operator(
            "export.select_model_export_directory", text="Select export directory"
        )
        row = layout.row()
...
class VIEW3D_PT_export_mesh_inprops_subpanel(VIEW3D_PT_MuSkeMo, Panel):
    ...
    def draw(self, context):
        ...
        split2.operator(
            "export.export_mesh_inertial_props", text="Export mesh inertial properties"
        )
...
## File export options
class VIEW3D_PT_export_options_subpanel(
    VIEW3D_PT_MuSkeMo, Panel
):  # class naming convention ‘CATEGORY_PT_name’
    ...
    def draw(self, context):
        ...
        row = self.layout.row()
        row.label(
            text="Default behavior is to export as a csv file, using ',' as the delimiter."
        )
        row = self.layout.row()
        row.label(
            text="You can export to other text filetypes (eg. txt, json) and customize the delimiter"
        )
