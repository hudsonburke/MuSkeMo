...
def copy_object(
    object, new_name
):  ## inputs: object (with a mesh) to be copied, string of the desired name
    object_copy = object.copy()  # copy the object
    object_copy.name = new_name
    object_copy.data = (
        object_copy.data.copy()
    )  # originally the data is linked to the original object. Here we give the copy its own instance of object data
    return object_copy
...
class SelMeshesInertialProperties(Operator):
    ...
    def execute(self, context):
        from .. import (
            inertial_properties,
        )  # import the function that computes inertial properties from a mesh

        density = bpy.context.scene.muskemo.segment_density  # user assigned

        sel_obj = (
            bpy.context.selected_objects.copy()
        )  # should be the source objects (e.g. skin outlines) that we want to compute inertial properties for. We're copying this because the inertial properties func resets the selections

        # throw an error if no objects are selected
        if len(sel_obj) == 0:
            self.report(
                {"ERROR"},
                "No meshes selected. You must select at least 1 target mesh to compute inertial properties",
            )
            return {"FINISHED"}
        ...
        for s_obj in sel_obj:  # for all the selected objects, error check loop
            ...
            if (
                s_obj.type != "MESH"
            ):  # check if the type is 'MESH'. If not, throw an error and abort

                self.report(
                    {"ERROR"},
                    "Source object with the name '"
                    + s_obj.name
                    + "' is not a 'MESH'. This button computes inertial properties for meshes. If you're defining rigid bodies, use the Body Panel. Operation cancelled.",
                )
                return {"FINISHED"}
            ...
            if any(len(p.vertices) != 3 for p in s_obj.data.polygons):
                self.report(
                    ...
                    "Source object with the name '"
                    + s_obj.name
                    + "' has non-triangular mesh faces. You must manually triangulate this mesh before computing inertial properties. Operation cancelled.",
                )
            ...
            if any(len(errors) != 0 for errors in errors_list):
                self.report(
                    {"ERROR"},
                    s_obj.name
                    + " is not a solid (airtight) mesh, it has "
                    + str(len(edges_non_manifold))
                    + " non-manifold edges, "
                    + str(len(edges_non_contig))
                    + " non-contiguous edges, and "
                    + str(len(faces_error))
                    + " self-intersections. Repair this mesh first, eg. with the 3D-Print Toolbox in Blender. Operation cancelled.",
                )
        ...
        if (
            len(total_mesh_errors) > 0
        ):  # if we caught non-triangular meshes, or bad meshes, abort operation
            return {"FINISHED"}
        ...
        for (
            ...
            sel_obj
        ):  # for all the selected objects, assign density and compute inertial properties
...
class CollectionMeshInertialProperties(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report(
                {"ERROR"}, "Collection with the name '" + colname + "' does not exist."
            )
            return {"FINISHED"}

        col_obj = [
            obj for obj in bpy.data.collections[colname].objects
        ]  # collection objects
        ...
        if len(col_obj) == 0:
            self.report(
                {"ERROR"},
                "Target collection is empty. Type the name of the collection that contains the target meshes",
            )
            return {"FINISHED"}
        ...
        for s_obj in col_obj:  # for all the selected objects, error check loop
            ...
            if (
                ...
                self.report(
                    ...
                    "Source object with the name '"
                    + s_obj.name
                    + "' is not a 'MESH'. Remove it from collection '"
                    + colname
                    + "' and try again. If defining rigid bodies, use the Body Panel instead.",
                )
                return {"FINISHED"}
...
class CollectionConvexHull(Operator):
    ...
    def execute(self, context):
        ...
        skel_colname = (
            muskemo.skeletal_mesh_collection
        )  # Collection that contains the skeletal meshes
        CH_colname = (
            muskemo.convex_hull_collection
        )  # Collection that will contain the convex hulls
        ...
        CH_coll = bpy.data.collections[
            CH_colname
        ]  # Collection which will recieve the scaled  hulls

        if (
            CH_colname not in bpy.context.scene.collection.children
        ):  # if the collection is not yet in the scene
            bpy.context.scene.collection.children.link(CH_coll)  # add it to the scene

        # Make sure the collection is active
        bpy.context.view_layer.active_layer_collection = (
            bpy.context.view_layer.layer_collection.children[CH_colname]
        )

        skel_coll = bpy.data.collections[skel_colname]

        meshes = [
            x for x in skel_coll.objects if "MESH" in x.id_data.type
        ]  # get the objects in target coll, if the data type is a 'MESH'
        ##### Generate minimal convex hulls #####
        ...
        for mesh in meshes:  # loop through each mesh
            ...
            if (
                CH_name not in bpy.data.objects
            ):  # check that the convex hull doesn't already exist
...
class AddSegmentOperator(Operator):
    ...
    )  # This will store which subpanel called the operator
...
class RemoveSegmentOperator(Operator):
    ...
    def execute(self, context):
        if self.mode == "arithmetic":
            ...
            context.scene.muskemo.whole_body_mass_logarithmic_parameters.remove(
                self.index
            )

        elif self.mode == "logarithmic_segmentinprops":
            context.scene.muskemo.segment_inertial_logarithmic_parameters.remove(
                self.index
            )
...
class ExpandConvexHullCollectionOperator(Operator):
    ...
    def execute(self, context):
        ...
        eCH_colname = (
            muskemo.expanded_hull_collection
        )  # Collection that will contain the expanded convex hulls

        apply_bias_correction = (
            muskemo.apply_bias_correction
        )  # bool for if we should correct for retransformation bias using 10**(log(10)/2 * MSE)

        if CH_colname not in bpy.data.collections:
            self.report(
                {"ERROR"},
                "A collection with the name '"
                + CH_colname
                + "' does not exist. Which collection contains the convex hulls? Type that into the 'Convex hull collection' field",
            )
            return {"FINISHED"}
        ...
        eCH_coll = bpy.data.collections[
            eCH_colname
        ]  # Collection which will recieve the scaled  hulls

        if (
            eCH_colname not in bpy.context.scene.collection.children
        ):  # if the collection is not yet in the scene
            bpy.context.scene.collection.children.link(eCH_coll)  # add it to the scene

        # Make sure the collection is active
        bpy.context.view_layer.active_layer_collection = (
            bpy.context.view_layer.layer_collection.children[eCH_colname]
        )

        ## get names of the convex hull objects
        CH_name = [
            x.name
            for x in bpy.data.collections[CH_colname].objects
            if "MESH" in x.id_data.type
        ]  # get the name for each object in collection 'Convex Hulls', if the data type is a 'MESH'
        ...
        if arithmetic_or_logarithmic == "arithmetic":
            ...
            log_MSEs = [
                x["log_MSE"] for x in logarithmic_parameters
            ]  # mean squared errors

        # inform the user about the behaviour
        if (
            len(segment_types) == 1 and segment_types[0] == "whole_body"
        ):  # if we do the same expansion for all segments
            if arithmetic_or_logarithmic == "arithmetic":
                self.report(
                    {"WARNING"},
                    "Expanding the whole body with a constant factor of "
                    + f"{expansions[0]:.3f}"
                ...
                self.report(
                    {"WARNING"},
                    "Logarithmic whole body expansion currently not yet supported. Eventually, this mode will sum all the volumes together, determine the whole-body expansion factor, and then expand on a per-segment basis.",
                )

        for h in range(
            len(CH_name)
        ):  # loop through each mesh in 'Convex Hulls' collection
            ...
            if (
                ...
                if any(
                    s in CH_name[h] for s in segment_types
                ):  # check if any of the segment types are in the object's name
                    segment_type = [s for s in segment_types if s in CH_name[h]][
                        0
                    ]  # check which entry in labels matches the current segment name. Make sure all objects in Collection "Skeleton" contain an entry from labels
                    ...
                    self.report(
                        ...
                        + CH_name[h]
                        + "' does not contain any of the segment types in its name. Skipping this object during expansion.",
                    )
                    continue
            ...
            if (
                eCH_name in bpy.data.objects
            ):  # ensure a unique name, just in case the name is already in use.
                base_name = eCH_name
                counter = 1
            ...
            bpy.ops.object.select_all(
                action="DESELECT"
            )  # Deselect all, then select desired object
            obj.select_set(True)  # select the hull
            bpy.ops.object.transform_apply()
            mass, CoM_book, mass_I_com, vol_before, volumetric_I_com = (
                inertial_properties(obj)
            )
            vol_mirrored = (
                vol_before  # vol mirrored gets overwritten if it's an axial segment
            )
            #### symmetrization ####
            if any(
                [s in obj.name for s in ["head", "neck", "torso", "tail"]]
            ):  # If head, neck, torso or tail are in the name
                bpy.ops.object.select_all(
                    action="DESELECT"
                )  # Deselect all, then select desired object
                obj.select_set(True)  # select the hull
                ...
                obj.modifiers.new(
                    "mirror", "MIRROR"
                )  # add a mirror modifier to generate a symmetric hull
                ...
                mass, CoM_book, mass_I_com, vol_mirrored, volumetric_I_com = (
                    inertial_properties(obj)
                )  # this recomputes the volume of the mirrored duplicated object, which is probably larger
            ...
            ind = segment_types.index(
                segment_type
            )  # index number (starting at zero) in labels of the current segment

            if arithmetic_or_logarithmic == "arithmetic":

                expansion_factor = expansions[
                    ind
                ]  # how much are you scaling the volume
                ...
                if (
                    not apply_bias_correction
                ):  # if apply_bias_correction is False, we set MSE to zero
                    MSE = 0

                uncorrected_vol = (
                    10**intercept * vol_before**slope
                )  # volume without MSE correction
                MSE_corr_vol = uncorrected_vol * 10 ** (
                    log(10) / 2 * MSE
                )  # MSE corrected volume
                expansion_factor_allo = MSE_corr_vol / vol_before
                expansion_factor = expansion_factor_allo

                self.report(
                    {"WARNING"},
                    "Logarithmic per-segment expansion is currently experimental, pending clarification from the lead authors. It is recommended you double-check the acquired results",
                )
                # return {'FINISHED'}

            correction_factor = (
                vol_mirrored / vol_before
            )  # correct for symmetrization. If the segment isn't symmetrized, this equals 1
            # scale factor
            sf = sqrt(
                expansion_factor / correction_factor
            )  # square root to scale in two directions, correcting for the increased volume due to mirroring

            #### directional scaling ####
            if any(
                [
                    s in obj.name
                    for s in ["head", "neck", "torso", "tail", "forearm", "hand", "toe"]
                ]
            ):
                obj.scale = (1, sf, sf)
            ...
            bpy.ops.object.transform_apply()
            mass, CoM_book, mass_I_com, vol_after, volumetric_I_com = (
                inertial_properties(obj)
            )  # this recomputes the volume of the mirrored duplicated object, which is probably larger
...
class WholeBodyMassFromConvexHullsOperator(Operator):
    ...
    def draw(self, context):
        ...
        layout.prop(
            self, "total_body_mass", text="Total body mass (kg)"
        )  # Copyable float property
    ...
    def execute(self, context):
        ...
        convex_hulls = [
            ...
            for x in bpy.data.collections[CH_colname].objects
            if "MESH" in x.id_data.type
        ]  # get each object in collection 'Convex Hulls', if the data type is a 'MESH'
        volumes = []
        # get the volumes
        ...
        if len(logarithmic_parameters) != 1:
            self.report(
                {"ERROR"},
                "This button only works with a single allometric equation for the whole body. You can only define one segment, and it has to be named 'whole_body'.",
            )
            return {"FINISHED"}
        ...
        total_body_mass = (
            10**log_intercept * vol**log_slope * 10 ** (log(10) / 2 * log_MSE)
        )

        if (
            muskemo.mass_from_CH_template_logarithmic
            == "Wright 2024 Logarithmic Tetrapods"
        ):
            density = muskemo.segment_density
            total_body_mass = (
                total_body_mass * density**log_slope
            )  # Wright's equation assumes volume is multiplied by density before placing it in the power function.
...
## Main panel
class VIEW3D_PT_inertial_prop_panel(
    VIEW3D_PT_MuSkeMo, Panel
    ...
    def draw(self, context):
        ...
        row = self.layout.row()
        row.operator(
            "inprop.inertial_properties_selected_meshes",
            text="Compute for selected meshes",
        )
        ...
        row = self.layout.row()
        row.operator(
            "inprop.inertial_properties_collection",
            text="Compute for all meshes in collection",
        )
...
class VIEW3D_PT_convex_hull_subpanel(
    VIEW3D_PT_MuSkeMo, Panel
):  # class naming convention ‘CATEGORY_PT_name’
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator(
            "inprop.generate_convex_hull_collection", text="Generate convex hulls"
        )
...
class VIEW3D_PT_expand_convex_hulls_arith_subpanel(VIEW3D_PT_MuSkeMo, Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("inprop.add_segment", text="Add Segment", icon="ADD").mode = (
            "arithmetic"
        )
        ### dynamically sized panel
        ...
        row = self.layout.row()
        op = row.operator(
            "inprop.expand_convex_hull_collection", text="Expand convex hulls"
        )
...
class VIEW3D_PT_whole_body_mass_from_convex_hull_subpanel(VIEW3D_PT_MuSkeMo, Panel):
    ...
    def draw(self, context):
        ...
        row = self.layout.row()
        op = row.operator(
            "inprop.compute_whole_body_mass_ch", text="Compute whole body mass"
        )
...
class VIEW3D_PT_segment_inprops_from_convex_hull_subpanel(VIEW3D_PT_MuSkeMo, Panel):
    ...
    def draw(self, context):
        ...
        row.template_list(
            ...
            "",
            muskemo,
            "segment_inertial_logarithmic_parameters",
            muskemo,
            "segment_index",
        )

        # Add segment button
        box.operator("inprop.add_segment", text="Add Segment", icon="ADD").mode = (
            "logarithmic_segmentinprops"
        )
