...
class CreateNewBodyOperator(Operator):
    ...
    bl_label = (
        "Creates a new body at the origin. Choose a unique name and press the button."
    )
    bl_description = (
        "Creates a new body at the origin. Choose a unique name and press the button."
    )
    ...
    def execute(self, context):
        ...
        colname = (
            bpy.context.scene.muskemo.body_collection
        ...
        try:
            ...
            create_body(
                name=name, self=self, size=rad, is_global=True, collection_name=colname
            )
            ...
            self.report(
                ...
                + name
                + " already exists, please choose a different (unique) name",
            )
...
class UpdateLocationFromCOMOperator(Operator):
    ...
    def execute(self, context):
        ...
        )  # should be the source objects (e.g. skin outlines) with precomputed inertial parameters
        ...
        if len(sel_obj) > 1:
            self.report(
                {"ERROR"},
                "Too many objects selected. Select a single body and try again.",
            )
            return {"FINISHED"}
        ...
        if len(sel_bodies) != 1:
            self.report(
                {"ERROR"},
                "The object you selected is not a body. Select a single body and try again.",
            )
            return {"FINISHED"}
        ...
        if np.isnan(COM).any():
            self.report(
                ...
                "Body with the name '"
                + target_body.name
                + "' has NANs in de COM property, define a COM first",
            )
        else:
            ...
            if (
                target_body["local_frame"] != "not_assigned"
            ):  # if the target body has a local frame assigned
                ...
                gRb = (
                    frame.matrix_world.to_3x3()
                )  # rotation matrix of the frame, local to global
                bRg = gRb.copy()
                bRg.transpose()

                frame_or_g = (
                    frame.matrix_world.translation
                )  # frame origin in global frame
                COM_g = Vector(target_body["COM"])  # COM loc in global frame

                relCOM_g = (
                    COM_g - frame_or_g
                )  # Relative COM location from the local frame origin, aligned in global frame
                relCOM_b = (
                    bRg @ relCOM_g
                )  # COM of the body, expressed in the local frame

                target_body["COM_local"] = list(relCOM_b)  # set COM in local frame

                MOI_glob_vec = target_body[
                    "inertia_COM"
                ]  # vector of MOI about COM, in global frame. Ixx Iyy Izz Ixy Ixz Iyz
                MOI_g = Matrix(
                    (
                        (
                            MOI_glob_vec[0],
                            MOI_glob_vec[3],
                            MOI_glob_vec[4],
                        ),  # MOI tensor about COM in global
                ...
                MOI_b = (
                    bRg @ MOI_g @ gRb
                )  # Vallery & Schwab, Advanced Dynamics 2018, eq. 5.53

                MOI_b_vec = [
                    MOI_b[0][0],  # Ixx, about COM, in local frame
                    MOI_b[1][1],  # Iyy
                    MOI_b[2][2],  # Izz
                    MOI_b[0][1],  # Ixy
                    MOI_b[0][2],  # Ixz
            ...
            if len(children) == 0:  # if the object has no children
                ...
                target_body["default_pose"] = list(
                    target_body.matrix_world
                )  # track the default pose to ensure the exported values are in the same pose
...
class AssignInertialPropertiesOperator(Operator):
    ...
    def execute(self, context):
        ...
        if len(sel_obj) <= 1:
            self.report(
                {"ERROR"},
                "Not enough objects selected. You must select the target body and 1+ source objects with mass properties precomputed in the Inertial properties panel.",
            )
            return {"FINISHED"}
        ...
        if len(sel_bodies) == 0:
            self.report(
                {"ERROR"},
                "No bodies selected. You must select select a body to which you would like to assign inertial properties",
            )
            return {"FINISHED"}
        ...
        if len(sel_bodies) > 1:
            self.report(
                {"ERROR"},
                "Multiple bodies selected. You must select select one body to which you would like to assign inertial properties",
            )
            return {"FINISHED"}
        ...
        for s_obj in source_objects:
            try:
                ...
                self.report(
                    ...
                    + s_obj.name
                    + "' has no precomputed inertial properties. Compute these first in the Inertial properties panel.",
                )
                return {"FINISHED"}
        ...
        for s_obj in source_objects:
            if s_obj.matrix_world != Matrix(s_obj["default_pose"]):
                self.report(
                    ...
                    + s_obj.name
                    + "' were computed in a different pose than the current pose. Reset the model to the default pose, or recompute the inertial properties. Operation cancelled",
                )
                return {"FINISHED"}

        if (
            len(source_objects) == 1
        ):  # trivial case, one source object provides the target body inertial parameters
            ...
            inertia_COM_indiv = [
                x["inertia_COM"] for x in source_objects
            ]  # inertia about source object COM, per body

            COM = (
                sum([np.array(x["COM"]) * x["mass"] for x in source_objects]) / mass
            )  # combined COM

            r_CO = [
                np.array(x["COM"]) - COM for x in source_objects
            ]  ## vector from combined COM to body COM, list of one for each body
            ...
            for b in range(len(masses)):
                Ixx_co.append(
                    masses[b] * (r_CO[b][1] ** 2 + r_CO[b][2] ** 2)
                    + inertia_COM_indiv[b][0]
                )
                Iyy_co.append(
                    masses[b] * (r_CO[b][0] ** 2 + r_CO[b][2] ** 2)
                    + inertia_COM_indiv[b][1]
                )
                Izz_co.append(
                    masses[b] * (r_CO[b][0] ** 2 + r_CO[b][1] ** 2)
                    + inertia_COM_indiv[b][2]
                )

                Ixy_co.append(
                    -masses[b] * (r_CO[b][0] * r_CO[b][1]) + inertia_COM_indiv[b][3]
                )
                Ixz_co.append(
                    -masses[b] * (r_CO[b][0] * r_CO[b][2]) + inertia_COM_indiv[b][4]
                )
                Iyz_co.append(
                    -masses[b] * (r_CO[b][1] * r_CO[b][2]) + inertia_COM_indiv[b][5]
                )
        ...
        if (
            ...
            target_body["inertia_COM_local"] = (
                MOI_b_vec  # add moment of inertia in local frame to the body
            )
...
class AttachVizGeometryOperator(Operator):
    ...
    def execute(self, context):
        ...
        if len(sel_obj) <= 1:
            self.report(
                {"ERROR"},
                "Not enough objects selected. You must select the target body and 1+ visual geometry meshes to attach to the target body.",
            )
            return {"FINISHED"}
        ...
        if len(sel_bodies) == 0:
            self.report(
                {"ERROR"},
                "No bodies selected. You must select a body to which you would like to attach visual geometry meshes",
            )
            return {"FINISHED"}
        ...
        if len(sel_bodies) > 1:
            self.report(
                {"ERROR"},
                "Multiple bodies selected. You must select select one body to which you would like to attach visual geometry meshes",
            )
            return {"FINISHED"}
        ...
        error_check = (
            []
        )  # check if there are non-mesh data types, visualization geometry can only be of the type mesh

        for (
            ob
        ) in (
            geom_objects_MuSkeMo
        ):  # if any of the prospective geom objects are already a different muskemo type (e.g., joint), don't allow geometry assignment
            if ob["MuSkeMo_type"] != "GEOMETRY":
                ...
                self.report(
                    ...
                    "The selected object '"
                    + ob.name
                    + "' is of the MuSkeMo type '"
                    + ob["MuSkeMo_type"]
                    + "', which cannot act as visualization geometry. Operation cancelled.",
                )
        ...
        for geom in geom_objects:
            if geom.type != "MESH":
                ...
                self.report(
                    ...
                    "The selected object '"
                    + geom.name
                    + "' is not a 'MESH'. Only meshes can act as visualization geometry. Operation cancelled.",
                )
        ...
        if geom_colname not in bpy.data.collections:
            ...
            self.report(
                ...
                + geom_colname
                + "' doesn't exist yet, automatically creating it. Visualization geometry will be moved to this collection.",
            )
        ...
        if (
            geom_colname not in bpy.context.scene.collection.children
        ...
        bpy.context.view_layer.active_layer_collection = (
            bpy.context.view_layer.layer_collection.children[geom_colname]
        )
        ...
        for geom in geom_objects:
            ...
            all_MuSkeMo_objects = [
                obj for obj in bpy.data.objects if "MuSkeMo_type" in obj
            ]
            all_bodies = [
                obj for obj in all_MuSkeMo_objects if obj["MuSkeMo_type"] == "BODY"
            ]

            for obj in [
                obj for obj in all_bodies if obj != target_body
            ]:  # for all the bodies in the scene that aren't the target body
                ...
                if geom_relpath in obj["Geometry"]:
                    self.report(
                        ...
                        + geom.name
                        + "' is already attached to a different body with the name '"
                        + obj.name
                        + "'. Detach it from that body first. Skipped this geometry object.",
                    )
                    skip_geom = True
                    break
            ...
            if geom_relpath in target_body["Geometry"]:
                self.report(
                    ...
                    "The selected geometry '"
                    + geom.name
                    + "' is already attached to target body with name '"
                    + target_body.name
                    + "'. Skipped this geometry object.",
                )
                continue
            ...
            try:
                ...
                geom["MuSkeMo_type"] = (
                    "GEOMETRY"  # to inform the user what type is created
                )
                geom.id_properties_ui("MuSkeMo_type").update(
                    description="The object type. Warning: don't modify this!"
                )

            geom["Attached to"] = target_body.name
            geom.id_properties_ui("Attached to").update(
                description="The body that this geometry is attached to"
            )
            ...
            if matname not in bpy.data.materials:
                ...
                matnode_tree.nodes["Principled BSDF"].inputs[
                    "Base Color"
                ].default_value = bone_color
...
class DetachVizGeometryOperator(Operator):
    ...
    def execute(self, context):
        ...
        if len(sel_obj) < 1:
            self.report(
                {"ERROR"}, "No objects selected. You must select 1+ geometry objects"
            )
            return {"FINISHED"}

        muskemo_objects = [obj for obj in sel_obj if "MuSkeMo_type" in obj]
        geom_objects = [
            obj for obj in muskemo_objects if obj["MuSkeMo_type"] == "GEOMETRY"
        ]

        if len(geom_objects) < 1:  # if less than 1 object is in the geometry collection
            self.report(
                {"ERROR"},
                "None of the selected objects are of the MuSkeMo type 'GEOMETRY'. Operation cancelled.",
            )
            return {"FINISHED"}
        ...
        for geom in geom_objects:
            ...
            if geom["Attached to"] != "no body":
                ...
                parent_body["Geometry"] = parent_body["Geometry"].replace(
                    geom_relpath + geom_delimiter, ""
                )  # remove the geom path from the body
                geom["Attached to"] = (
                    "no body"  # update the "attached to" state of the geometry
                )

                if not parent_body[
                    "Geometry"
                ]:  # if the geometry list is now empty, state so explicitly
                ...
                self.report(
                    ...
                    "Geometry file '"
                    + geom.name
                    + "' does not appear to be attached to a body. Geometry skipped.",
                )
        return {"FINISHED"}
